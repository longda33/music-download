...
def request_json(url, params=None, headers=None, timeout=60):
    error = None
...
def qq_search(title, artist):
    rows = request_json(QQ_API, {"msg": f"{title} {artist}", "type": "json"}, SOURCE_HEADERS)
    if not isinstance(rows, list):
        return None
    for row in rows:
        if not isinstance(row, dict) or not row.get("song_mid"):
            continue
        detail = request_json(QQ_API, {"mid": row["song_mid"]}, SOURCE_HEADERS)
...
def kuwo_search(title, artist):
    data = request_json(KUWO_API, {"msg": f"{title} {artist}", "page": 1, "limit": 60}, SOURCE_HEADERS)
    for row in (data.get("data", []) if isinstance(data, dict) else []):
        ...
        for params in ({"rid": row["rid"]}, {"msg": row["rid"]}):
            try:
                detail = request_json(KUWO_API, params, SOURCE_HEADERS)
...
def netease_search(title, artist):
    data = request_json(NETEASE_API, {"type": "search", "id": f"{title} {artist}", "limit": 30, "page": 1, "server": "netease"}, SOURCE_HEADERS)
...
def existing_file(auth, filename, size):
    ...
    for response in root.findall("d:response", ns):
        ...
        if name == filename and abs(int(length or 0) - size) <= SIZE_TOLERANCE:
            return True
    return False
...
def main():
    ...
    for original in songs:
        ...
        if not found:
            ...
            continue
        filename = safe_name(f"{found['title']} - {found['artist']}.flac")
        local = work / filename
        try:
            ...
            if existing_file(auth, filename, actual):
